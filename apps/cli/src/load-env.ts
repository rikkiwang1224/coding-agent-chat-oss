...
/** Load nearest `.env` when CLI env vars are not set (does not override existing env). */
export function loadCliEnv(startDir?: string): void {
  if (loaded) return;
  loaded = true;
...
// For tests
export function resetCliEnvLoader(): void {
  loaded = false;
}
